...
namespace zx{
    // 创建一个全局的Logger对象，用于日志记录
    zx::Logger::ptr g_logger=zx::LoggerManger::LoggerMgr::GetInstance()->getLogger("system");
    // 定义一个原子的uint64_t变量，用于生成唯一的协程ID
    static std::atomic<u_int64_t> s_fiber_id{0};
    // 定义一个原子的uint64_t变量，用于跟踪当前活动的协程数量
    static std::atomic<u_int64_t> s_fiber_count{0};
    // 定义线程局部存储，用于存储指向当前协程的指针
    static thread_local Fiber* t_fiber =nullptr;
    // 定义线程局部存储，用于存储指向当前线程的主协程智能指针
    ...
    void Fiber::call(){
        SetThis(this);
        m_state=EXEC;
        //ZX_ASSERT(GetThis()==t_threadFiber);
